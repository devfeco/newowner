...
export default function GoogleHandlerPage() {
  ...
  const [message, setMessage] = useState('Google ile giriş işleniyor...')
  const [error, setError] = useState<string | null>(null)
  
  useEffect(() => {
    const processGoogleAuth = async () => {
      try {
        console.log('Google handler sayfası yüklendi')
        ...
        console.error('Google handler hatası:', err)
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <h1 className="text-2xl font-bold text-center mb-6">Google Giriş</h1>
